...
That's a nice graph showing not only levels, but whether the person is an activist (gold). For simplicity, I didn't make 'activist' an aesthetic. Now, maybe it would be interesting to make an activist layer after all, so the circles would nest well. 
...
women_graph_lvl3 <- women %>% 
  filter(!grepl("activist", role, ignore.case = TRUE)) %>% 
  select(name, category) %>% 
  rbind(data.frame(name = "Activist", category = NA)) %>% 
  mutate(level = 3, activist = FALSE)

women_graph_lvl4 <- women %>% 
  filter(grepl("activist", role, ignore.case = TRUE)) %>% 
  select(name, category) %>% 
  mutate(level = 4, activist = TRUE)
  

women_graph_vertices <- rbind(women_graph_lvl1, 
                              women_graph_lvl2, 
                              women_graph_lvl3,
                              women_graph_lvl4)

women_graph_activist_lyr <- women %>% 
  filter(grepl("activist", role, ignore.case = TRUE)) %>% 
  select(to = name) %>% 
  mutate(from = "Activist") %>% 
  unique()
...
women_graph_df <- women %>% 
  ...
  unique() %>% 
  mutate(role = NA) %>% 
  rbind(select(women, from = category, to = name, role)) %>% 
  mutate(
    to = case_when(
      grepl("activist", role, ignore.case = TRUE) ~ "Activist",
      TRUE ~ to
    )
  ) %>%
  select(from, to) %>%
  rbind(women_graph_activist_lyr)
...
I'm getting a multiple parent error. Some end point has more than one parent. I think it doesn't like the nesting of the 'activist' layer. The 'activist' groupings have multiple parents -- ah. I need to differentiate between the different 'activist' types. I can append the name I guess. This is getting pretty hairy; I'm not liking it too much. Maybe it's time for a redesign. I'm moving into an R file. 
